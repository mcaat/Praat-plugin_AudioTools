...
form Random Spectral Variation (Optimized)
    ...
    comment (each bin multiplied by center + depth * randomUniform(min, max))
...
writeInfoLine: "=== RANDOM SPECTRAL VARIATION OPTIMIZATION ==="
...
appendInfoLine: "  Variation center: ", variation_center
appendInfoLine: "  Variation depth: ", variation_depth
appendInfoLine: "  Variation range: ", variation_min, " to ", variation_max
...
for i to num_chunks
    ...
    # Apply random spectral variation (using explicit string concatenation)
    formula$ = "if col < " + string$(frequency_cutoff) + " then self[1, col] * (" + string$(variation_center) + " + " + string$(variation_depth) + " * randomUniform(" + string$(variation_min) + ", " + string$(variation_max) + ")) else self[1, col] fi"
    Formula: formula$
...
# STEP 7: Finalize
selectObject: resultID
outName$ = originalName$ + "_spectral_varied"
Rename: outName$
