...
form Exponential Low-Pass Filter (Optimized)
...
writeInfoLine: "=== GENTLE LOW-PASS OPTIMIZATION ==="
...
appendInfoLine: "  Filter cutoff: ", frequency_cutoff, " Hz"
...
for i to num_chunks
    ...
    # Apply exponential low-pass filter (using explicit string concatenation)
    formula$ = "if col < " + string$(frequency_cutoff) + " then self[1, col] * exp(-col / " + string$(decay_rate) + ") else self[1, col] fi"
    Formula: formula$
...
# STEP 7: Finalize
selectObject: resultID
outName$ = originalName$ + "_exp_lowpass"
Rename: outName$
