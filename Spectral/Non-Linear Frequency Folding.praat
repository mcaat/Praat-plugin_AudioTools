...
form Spectral Knots - Non-Linear Frequency Folding (Optimized)
...
writeInfoLine: "=== NON-LINEAR FREQUENCY FOLDING OPTIMIZATION ==="
...
appendInfoLine: "  Low freq threshold: ", low_freq_threshold, " Hz"
appendInfoLine: "  Folding period: ", folding_period, " Hz"
appendInfoLine: "  Sine modulation divisor: ", sine_modulation_divisor
appendInfoLine: "  Cosine modulation divisor: ", cosine_modulation_divisor
...
for i to num_chunks
    ...
    # Apply spectral knots with non-linear frequency folding (using explicit string concatenation)
    formula$ = "if col < " + string$(low_freq_threshold) + " then self[1, col] else self[1, abs(col - 2 * round(col / " + string$(folding_period) + ") * " + string$(folding_period) + ")] * (sin(col / " + string$(sine_modulation_divisor) + ") + cos(col / " + string$(cosine_modulation_divisor) + ")) ^ 2 fi"
    Formula: formula$
